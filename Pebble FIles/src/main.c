...
static void main_window_load(Window *window) {
	...
	text_layer_set_text(s_weather_layer, "Jeff");
...
static void tick_handler(struct tm *tick_time, TimeUnits units_changed) {
	...
	// Get weather update every 5 seconds
...
static void inbox_received_callback(DictionaryIterator *iterator, void *context) {
	...
	while (t != NULL) {
		...
		case KEY_TEMPERATURE:
		{
			...
			//if(strcmp("NaN", t->value->cstring) == 0)
			//  doPrint = false;
			break;
		}
		case KEY_CONDITIONS:
	...
	if (doPrint)
	{
		...
		text_layer_set_text(s_time_layer, weather_layer_buffer);
	}
